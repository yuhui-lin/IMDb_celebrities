...
var Draw = (function () {
    function Draw(svgId, type, json_path) {
        ...
        this.rScale = linear$2()
            ...
            .range([5, 40])
            .clamp(true);
        this.dScale = linear$2()
            .domain([0, 100]) // min and max value in our data array
            .range([250, 40])
            .clamp(true);
        this.sScale = linear$2()
            .domain([0, 100]) // min and max value in our data array
            .range([0.3, 1])
            .clamp(true);
        this.svgId = svgId;
        ...
        if (type == 'all') {
            ...
            this.partitionFunc = function (d) { return _this.color(d.partition.toString()); };
        }
        else if (type == 'movie') {
            this.weightFunc = function (d) { return d.w_movie; };
            this.degreeFunc = function (d) { return _this.rScale(d.degree_movie); };
            this.partitionFunc = function (d) { return _this.color(d.p_movie.toString()); };
        }
        else if (type == 'tvEps') {
            this.weightFunc = function (d) { return d.w_tvEps; };
            this.degreeFunc = function (d) { return _this.rScale(d.degree_tvEps); };
            this.partitionFunc = function (d) { return _this.color(d.p_tvEps.toString()); };
        }
        else {
    ...
    Draw.prototype.render = function () {
        var _this = this;
        var svg = select("svg" + '#' + this.svgId), width = +svg.attr("width"), height = +svg.attr("height");
        // name collision when using class, change simulation -> simulation1
        var simulation1 = simulation()
            .force("link", link()
            .id(function (d) { return d.id; })
            .distance(function (link) { return _this.dScale(_this.weightFunc(link)); })
            .strength(function (link) { return _this.sScale(_this.weightFunc(link)); }))
        ...
        json(this.json_path, function (error, graph) {
            ...
            node
                ...
                text.filter(function (td) { return td.name == d.name; })
                    .transition()
                    .ease(elasticOut)
                    .delay(100)
                    .duration(200)
                    .attr('display', 'on');
                link.filter(function (ld) { return ld.target == d || ld.source == d; })
                    .transition()
                    .delay(100)
                    .style('stroke', '#ff0202')
